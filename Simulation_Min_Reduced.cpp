...
int create_directory(TTree * t){
    ...
    t->Branch("w1px_candidate", &wp_candidate[0]); 
    t->Branch("w1py_candidate", &wp_candidate[1]); 
    t->Branch("w1pz_candidate", &wp_candidate[2]); 
    t->Branch("w2px_candidate", &wp_candidate[3]); 
    t->Branch("w2py_candidate", &wp_candidate[4]); 
    t->Branch("w2pz_candidate", &wp_candidate[5]); 
...
int fill_lab_values(){ // With references
    ...
    w1_unit_distance_lab_ref = w1_lab_ref.Vect().Unit(); 
    w2_unit_distance_lab_ref = w2_lab_ref.Vect().Unit();
	...
	l1_unit_distance_lab = l1_lab.Vect().Unit();
    l2_unit_distance_lab = l2_lab.Vect().Unit();

    return 0;
    }//fill_lab_values()

int add_smearing(float smearing_extent){ ///////////////////////////// TO ADD AT THE END
...
int ww_boost_to_com(float smearing_extent){  ///////////////////////////// TO ADD AT THE END WITH SMEARING
...
int average_momenta(){
	...
	number_of_candidates = vector_w1phi_lab_candidate.size(); // Only need to consider 1 since 2 is the same
...
int average_filter(float max_wwe_diff, float angle_limit){
    ...
    while(pass_cut == false){
        ...
        average_momenta(); // average from the selection that satisfied the cuts
...
string run_simulation(const int event_count, float max_wwe_diff, float angle_limit, const int max_phi, float smearing_extent, int i, float parameter, float parameter_count) {
    ...
    // File directory
    TFile * f = TFile::Open("Simulation_Output.root","RECREATE");
    ...
    max_events = event_count/7; // Turn in to maximum value of plot + padding

    successRate_out.push_back( successRate );
    //daw1_all_out.push_back( std::accumulate(daw1_all.begin(), daw1_all.end(), 0.0) / daw1_all.size() ); // Average angle difference w1
    //daw2_all_out.push_back( std::accumulate(daw2_all.begin(), daw2_all.end(), 0.0) / daw2_all.size() ); // Average angle difference w2
    //daw1_all.clear();
    //daw2_all.clear();
